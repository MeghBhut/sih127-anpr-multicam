# ANPR prototype — build instructions

SIH26127, multi-camera vehicle tracking. This document is the contract between 6 people.
Scope here is the **prototype only**: pretrained models, 2–3 test videos acting as cameras,
rows in a database, one map PNG. No fine-tuning, no web dashboard.
...
1. Freeze the function signatures in this document on day 1. Nobody changes one without telling the others.
2. Crops and frames are **BGR numpy arrays** everywhere (OpenCV default). Do not convert to RGB inside your module.
3. Anything unknown is `None` — never `""`, never `"unknown"`. `plate=None` means not read; `plate=""` means a bug.
4. Timestamps are **unix seconds (float)**, not frame numbers. The linker needs wall-clock time.
5. Every threshold lives in `config.py`. No magic numbers inside modules.
6. Day 1 everyone commits a **stub** that returns fake data matching their signature, so `main.py` runs end to end on day 2.
7. Only Megh merges to `main`. PRs need one review from whoever owns the module you touch.
...
### Done when
Two videos play through `main.py` and their timestamps overlap correctly on a printed timeline.

---

## Person 2 — detector.py  (medium)
...
### Done when
Running on one video prints a stable track ID per vehicle and one finished-track dict per vehicle that leaves.

---

## Person 3 — plate_reader.py  (hard)
...
### Done when
`read_plate` returns 10 chars with confidences on a clear car, and `None` on a blank frame.

---

## Person 4 — database.py + linker.py  (hard)
...
### Done when
Running on 3 videos produces links for vehicles that really did pass two cameras, and no links between different vehicles.

---

## Person 5 — visualizer.py  (easy, but owns every screenshot)
...
### Done when
One command regenerates all deck images from the current database.

---

## Person 6 (Megh) — main.py + config.py + plate_logic.py  (hardest)
...
### Done when
`python main.py --config config.py` produces annotated frames, sighting rows, link rows and the map PNG in one run.
...
If someone finishes early they help person 3. Plate reading quality decides how good every
screenshot looks.

## Limits to write down as you go

Person 5 collects these; they go on the feasibility slide, stated openly rather than hidden.
